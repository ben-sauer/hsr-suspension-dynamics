...
# # Draft Model, ultimately scrapped for above as inverse fft is much faster. Committing this
# # draft just this once for documentation
# def rough_input(t, start, stop, v, N = 2000, dx = None, A = 20.95, p = 0.8, seed = None):
#     """
#     Description:
#         Generate roughness profile for left and right rails. Meant to resemble physical rail.
#         PSD and randomness model is based on M. Podwórna's "Modeling of Random Vertical Irregularities of Railway Tracks".
...
#         start, stop (float)): Start/stop time of simulation [s]
#         v (float): Current velocity of train [s]
#         N (int): Resolution of frequency sampling
#         dx (int): Track Profile sampling step [m]
#         A (float): Vertical track irregularity parameter. See simulation.py for detailed information on standards.
#         p (float): Cross-level roughness. 1 = left and right rails identical. Decrease p to make left and right rail increasingly different.
#         seed (int): RNG seed (for reproducability)
...
#     def psd_function(omega):
#         S_k = A * omega_c**2 / ((omega**2 + omega_c**2)*omega**2)
#         return S_k
    
#     # Generate random vertical track irregularities
#     def random_irregularity(x):
#         z = 0.0
#         for n in (np.arange(0,N)):
#             omega_n = (omega_min + (n-0.5)) * domega # frequency index
#             phi_n = random.uniform(0, 2 * np.pi) # phase
#             a_k = math.sqrt(psd_function(omega_n) * domega)
#             z += 2 * a_k * np.cos(omega_n * x + phi_n)
#         return z

#     # Generate roughness profiles across rail space
#     r_0 = np.array([np.arange(0, L, dx), np.zeros(L)])
#     r_1 = np.array([np.arange(0, L, dx), np.zeros(L)])

#     for i in range(len(r_0[0])):
#         x = r_0[0][i]
#         r_0[1][i] = random_irregularity(x)

#     for i in range(len(r_1[0])):
#         x = r_1[0][i]
#         r_1[1][i] = random_irregularity(x)

#     r_L = r_0
#     r_R = p * r_0 + math.sqrt(1 - p**2) * r_1 

#     # Translate to time domain for inputs
#     U = np.zeros((len(t), 8))

#     U[:,0] = np.interp(v * t, r_L[0], r_L[1])   # Front Left displacent
#     U[:,4] = np.gradient(U[:,0], t)               # Front Left velocity
#     U[:,1] = np.interp(v * t, r_R[0], r_R[1])   # Front Right displacent
#     U[:,5] = np.gradient(U[:,1], t)               # Front Right velocity

#     U[:,2] = np.interp(v * t, r_L[0], r_L[1])   # Rear Left displacent
#     U[:,6] = np.gradient(U[:,2], t)               # Rear Left velocity
#     U[:,3] = np.interp(v * t, r_R[0], r_R[1])   # Rear Right displacent
#     U[:,7] = np.gradient(U[:,3], t)               # Rear Right velocity
